...
# gsutil ls gs://gatk-bam |grep 'bam$' > bamList.txt
# OR
# gsutil ls gs://gatk-bam |grep -H '.*bam$' |sed -e "s|.*\(gs.*$\)|\1|"  > bamList.txt
#------------------------------------------------------------------------------
"""
...
with open(listBAM, 'r') as f:
    for line in f:
        ...
        newName = "{}/{}.head.{}".format(tgPath, nameList[0], nameList[1])
...
#-- Write input & output mapping file
mapBAM = listBAM.split('/')
mapBAM = "{}/mapBAM-addPL.txt".format('/'.join(mapBAM[:len(mapBAM)-1]))
...
for i in range(len(inBAM)):
    ...
    print(cmt)
    dsub.headAddPL(prjName=prjName, inFile=ibam, outFile=obam, scriptPath=oScr, Logs=logPath)
    print('\n')
