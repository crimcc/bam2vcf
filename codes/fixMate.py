"""
# Purpose     : To submit multiple Picard CleanSam jobs via dsub
# Descriptions:
#  - Codes contain functions to submit jobs via Google Cloud 'dsub'
#  - This codes must be run after running 'cleanSam.py' that correct known issue of BAM files for GATK downstream anaysis
#
...
# gsutil ls gs://my-bam |grep 'bam$' > cleanBamList.txt
# OR
# gsutil ls gs://my-bam |grep -H '.*bam$' |sed -e "s|.*\(gs.*$\)|\1|"  > cleanBamList.txt
#------------------------------------------------------------------------------
"""
...
with open(listBAM, 'r') as f:
    for line in f:
        ...
        newName = "{}/{}.fixmate.{}".format(tgPath, '.'.join(nameList[:(len(nameList)-1)]), nameList[len(nameList)-1])
...
mapBAM = "{}/mapBAM_clean-fixmate.txt".format('/'.join(mapBAM[:len(mapBAM)-1]))
...
for i in range(len(inBAM)):
    ...
    print(cmt)
    dsub.FixMate(prjName=prjName, inFile=ibam, outFile=obam, scriptPath=oScr, Logs=logPath)
    print('\n')
