...
# gsutil ls gs://my-bam |grep 'bam$' > bam_idx.txt
# OR
# gsutil ls gs://my-bam |grep -H '.*bam$' |sed -e "s|.*\(gs.*$\)|\1|"  > bam_idx.txt
#------------------------------------------------------------------------------
"""
...
with open(listBAM, 'r') as f:
    for line in f:
        ...
        newName = "{}/{}.unmap.{}".format(tgPath, '.'.join(nameList[:(len(nameList)-1)]), nameList[len(nameList)-1])
...
mapBAM = "{}/unmapBAM.txt".format('/'.join(mapBAM[:len(mapBAM)-1]))
...
for i in range(len(inBAM)):
    ...
    print(cmt)
    dsub.UnmapBam(prjName=prjName, inFile=ibam, outFile=obam, scriptPath=oScr, Logs=logPath)
    print('\n')
